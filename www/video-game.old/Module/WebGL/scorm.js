...
var output = console; // output can be set to any object that has a log(string) function
...
{  
   ...
   else
   {
      ...
      if (error.code != _NoError.code)
      {
		 ...
		 SendMessage(objectname, callbackname, result+"" + "|" + error.code + "|" + error.string + "|" + randomnumber);
      }else
	  {
		SendMessage(objectname, callbackname, result+"" + "|" + "|" + "|" + randomnumber);
	  }
   }
   return result;
...
function doSetValue(identifier,value,objectname,callbackname,randomnumber)
{
   ...
   else
   {
      ...
      if (result.toString() != "true")
      {
		...
		SendMessage(objectname, callbackname, result.toString() + "|" + err.code + "|" + err.string + "|" + value + "|" + dotBindingName);
      }
	  else
	  {
		SendMessage(objectname, callbackname, result.toString() + "|"  + "|"  + "|" + randomnumber);
	  }
   }
...
function doIsScorm2004(objectname,callbackname,randomnumber)
{
   ...
   SendMessage(objectname, callbackname, versionIsSCORM2004 + "|"+"|"+"|" + randomnumber);
}
...
var scorm = (function(){
	return {
		foo: function(str){
			alert(str);
			SendMessage("GameObject", "fromweb", "message from web");
		}
	};
...
function OLDdoGetValue(identifier,objectname,callbackname,randomnumber)
{
   ...
   SendMessage(objectname, callbackname, "Hello from a web page!|"+randomnumber);
}
...
function OLDdoSetValue(identifier,value,objectname,callbackname,randomnumber)
{
	...
	SendMessage(objectname, callbackname, "Hello from a web page!"+randomnumber+"|"+randomnumber);
}